...
% fixation cross
p.crs.color                 = [0.4 0.4 0.4 1];    % color of fixation cross [0.5 0.5 0.5 1];
...
p.trig.tr_start         = 200;                   % trial start; main experiment
p.trig.tr_stop          = 201;                   % trial end; main experiment
p.trig.tr_cue_type      = [10 20 30];           % cue type: [RDK1 RDK2]
p.trig.type             = [1 2];                % number of events
p.trig.button           = 60;                   % button press
p.trig.event_type       = [111 112 113];        % target primed color, target second color, distractor
...
if p.flag_training
    ...
    while flag_trainend == 0 % do training until ended
        ...
        % brightr cross initially
        p_tr = p; p_tr.crs.color = [0.5 0.5 0.5 1];
        randmat.training{i_bl} = rand_FShiftPrime1of2(p_tr, RDK,  1);
